...
- **必须用单反引号行内代码**（写在一行内），严禁 ``` 三反引号代码块——代码块里的
  路径不会渲染为可点击。格式：完整文件路径 + 单个 `:行号`（相对工作区根，行号 1 起算），
  如 `attachments/20260916-1418-log0914/opt/zdocs/luoshu_log/NewDocServer_5/combined-2026-06-01.log:2737`。
  日志原文摘要写在行内代码外面的普通文本里，不要和路径挤进同一个代码块。
- **路径必须完整，严禁截短**：从 `attachments/<解压目录>/` 一级不落地写到最后一级文件，
  与文件系统实际位置逐字一致。只写 `TaskServer_2/combined-....log:123` 这类半截路径
  会导致点击后找不到文件、无法定位——**宁可长，不可短**。
  **交付前自检**：逐条 `ls` 校验引用路径（去掉 `:行号`）存在，不存在必须修正后再输出。
