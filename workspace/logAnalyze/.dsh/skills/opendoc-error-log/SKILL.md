...
- 有日志文件时，行内代码里**只写完整文件路径**（相对工作区根，与 present 交付的路径
  完全一致，如 `attachments/20260916-1418-log0914/opt/zdocs/luoshu_log/NewDocServer_5/combined-2026-06-01.log`），
  行号写在行内代码**外**的普通文本里（如「……第 2737 行」）。
  **不得**把 `:行号` 写进行内代码——带后缀会破坏路径匹配，点击跳转失效。
